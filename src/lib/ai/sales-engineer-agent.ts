...
export function buildLeadHandoffQualification(memory: ProjectMemory): SalesEngineerToolResult {
  ...
  const contactChecks = [
    { key: "name", label: "tên khách hàng", ready: Boolean(memory.name?.trim()) },
    { key: "phone", label: "số điện thoại", ready: phoneDigits.length >= 9 },
  ];
  const projectChecks = [
    { key: "service", label: "hạng mục", ready: Boolean(memory.service?.trim()) },
    {
      key: "location",
      label: "khu vực hoặc địa chỉ khảo sát",
      ready: Boolean(memory.location?.trim() || memory.surveyAddress?.trim()),
    },
    { key: "dimensions", label: "kích thước sơ bộ", ready: Boolean(memory.dimensions?.trim()) },
  ];
