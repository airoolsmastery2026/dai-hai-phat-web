...
export function HeroSection() {
  return (
    <section className="relative overflow-hidden bg-[var(--color-surface-dark)] py-24 text-white md:py-32 lg:min-h-[92vh]">
      ...
      <Container className="relative z-10">
        <div className="grid items-end gap-16 lg:grid-cols-[1.05fr_0.95fr]">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.3em] text-orange-300 backdrop-blur">
              <ShieldCheck className="h-4 w-4 text-[var(--color-primary)]" /> {COMPANY_CONFIG.shortName} • Industrial Engineering
            </div>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <Button href="#bao-gia">Request a consultation <ArrowRight className="h-4 w-4" /></Button>
              <Button href="#about" variant="secondary">Explore capabilities</Button>
            </div>
            <div className="mt-12 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {COMPANY_STATS.map((stat) => (
                <div key={stat.label} className="rounded-[var(--radius-lg)] border border-white/10 bg-white/10 p-4 backdrop-blur-sm">
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="rounded-[var(--radius-xl)] border border-white/10 bg-white/10 p-3 shadow-[var(--shadow-lg)] backdrop-blur">
