  ...
  return (
    <section id="about" className="border-b border-slate-200 bg-white py-20 lg:py-24">
      <Container className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#FF5722]">About</p>
          <h2 className="mt-3 text-3xl font-semibold leading-tight text-slate-900 sm:text-4xl">
            Đối tác thi công cơ khí và xây dựng đáng tin cậy cho công trình thương mại và công nghiệp.
          </h2>
          <p className="mt-6 text-lg leading-8 text-slate-600">
            {COMPANY_CONFIG.name} chuyên thi công kết cấu thép, nhà xưởng, mái che, hệ thống cầu trục và gia công cơ khí với quy trình thực hiện rõ ràng, đội ngũ kỹ thuật giàu kinh nghiệm và phương án tối ưu cho từng công trình.
          </p>
          <div className="mt-8 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700">
            <ShieldCheck className="h-4 w-4 text-[#FF5722]" /> Tư vấn kỹ thuật và thi công đúng chuẩn vận hành.
          </div>
        </div>
