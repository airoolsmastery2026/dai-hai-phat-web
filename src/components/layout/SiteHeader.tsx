      ...
      <div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between px-4 md:px-8">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-medium uppercase tracking-widest text-slate-500">C CƠ KHÍ & NỘI THẤT DESIGNER</span>
          </div>
        </Link>

        <nav aria-label="Điều hướng chính" className="hidden items-center gap-8 text-sm font-semibold text-slate-700 lg:flex">
          {NAV_ITEMS.map((item) => (
        ...
        <div className="hidden lg:flex">
          <a
            href="#bao-gia"
            className="rounded-lg px-6 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:brightness-110"
            style={{ backgroundColor: theme.colors.primary }}
          >
            Nhận Báo Giá
          </a>
      ...
      {mobileMenuOpen ? (
        <div className="border-t border-slate-200 bg-white px-4 py-4 lg:hidden">
          <div className="flex flex-col gap-3 text-sm font-semibold text-slate-700">
            ...
            <a href="#bao-gia" onClick={() => setMobileMenuOpen(false)} style={{ color: theme.colors.primary }}>
              Nhận Báo Giá
            </a>
          </div>
        </div>
