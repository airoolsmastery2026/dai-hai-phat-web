...
export function RawFinishedRevealStudio({ enabled }: RawFinishedRevealStudioProps) {
  ...
  const previews = [
    { label: "THÔ", url: rawUrl },
    { label: "HOÀN THIỆN", url: finishedUrl },
  ];
    ...
    <section className="space-y-[var(--space-5)]" aria-labelledby="raw-finished-title">
      <div>
        ...
        <p className="mt-[var(--space-2)] max-w-3xl text-sm leading-6 text-[var(--color-text-muted)]">
          Tải một ảnh hiện trạng hoặc ảnh hoàn thiện. AI dựng hai trạng thái cùng góc máy; Crossfade được tạo ngay trên thiết bị và không cần API video.
        </p>
      </div>

      {!enabled ? (
        <Alert title="Công cụ chưa sẵn sàng" tone="warning">
          Gemini chưa được cấu hình trên máy chủ. Không có khóa nhà cung cấp nào được nhập hoặc lưu trên trình duyệt.
        </Alert>
      ) : null}
            ...
            {previews.map((item) => (
              <div key={item.label}>
                ...
                <div className="relative aspect-video overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-background)]">
                  {item.url ? (
                    <NextImage src={item.url} alt={`Bản xem trước ${item.label.toLowerCase()}`} fill unoptimized sizes="(min-width: 1024px) 25vw, 50vw" className="object-cover" />
                  ) : (
